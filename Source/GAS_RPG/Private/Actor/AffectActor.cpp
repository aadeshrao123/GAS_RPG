...
void AAffectActor::OnOverlap(AActor* TargetActor)
{
...
void AAffectActor::OnEndOverlap(AActor* TargetActor)
{
	...
	if (InfiniteGameplayEffectRemovalPolicy == EEffectRemovalPolicy::RemoveOnEndOverlap)
	{
		...
		for (TTuple<FActiveGameplayEffectHandle, UAbilitySystemComponent*> HandlePair : ActiveEffectHandles)
		{
			...
			{
				TargetASC->RemoveActiveGameplayEffect(HandlePair.Key, 1); // For Removing our Active GameplayEffect
				HandlesToRemove.Add(HandlePair.Key);
			}
		}
		for (FActiveGameplayEffectHandle Handle : HandlesToRemove)
		{
			ActiveEffectHandles.FindAndRemoveChecked(Handle); // For removing the removed Active gameplay effect from our TMap @ActiveEffectHandles
		}
	}
}
